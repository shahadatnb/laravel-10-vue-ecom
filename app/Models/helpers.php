...
function NaveMenuUrl($item, $linkClass){
    ...
    return "<a class=\"{$linkClass} {$item->menu_class}\" href=\"{$url}\">{$item->lebel}</a>";
}
...
function SocialMenu($menu_id, $menuClass='menu', $listClass='', $lebel=false, $linkClass='nav-link'){
    $menu = Menu::where('menu_id',$menu_id)->first();
    if($menu):
        $nav = "<ul class=\"{$menuClass}\">";
        foreach($menu->menuItem as $item){
                $lebel = $lebel? $item->lebel : '';
                $nav .= "<li class=\"{$listClass}\"><a class=\"{$linkClass} {$item->menu_class}\" href=\"{$item->menu_url}\">{$lebel}</a></li>";
            }
        $nav .= '</ul>';
        return $nav;
    else:
