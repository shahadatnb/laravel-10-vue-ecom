...
{   
    ...
    public function settings(){
        $settings = Setting::where('category','basic')->pluck('value','name')->toArray();
        $munus = Menu::all();
        foreach($munus as $m){
            $settings['menus'][$m->menu_id] = MenuItem::with('subMenu')->withCount('subMenu')->where('menu_id',$m->id)->where('parent_id',null)->orderBy('sl')->orderBy('sl','ASC')->get()->toArray();
        }
        
        return $settings;
    }

    public function nenuUrl($item){
    ...
    public static function order_status(){
        return OrderStatus::where('status','Active')->orderBy('sl','ASC')->pluck('name','id')->toArray();
    }
    ...
    public function checkAssigned(){
        ...
        })->withCount('orders')->orderBy('orders_count','ASC')->first();

        return $users;
    }


    public function shippingMmethod(){
        $ss = ShippingRole::all();
        $shippings = [];
        foreach($ss as $s){
            $shippings[$s->id]=$s->title;
        }
        return $shippings;
    }

    public function shipingAmount($id = null)
    {
        $weight = 0;
        //return session('cart');
        if(count((array) session('cart')) > 0){
            $free_shipping_sum = array_reduce(session('cart'), function($carry, $item) {
                $carry += $item['free_shipping'];
                return $carry;
            });
            
            if($free_shipping_sum != 0){
                return 0;
            }
            
            if($id) {
                $shippingRole = ShippingRole::where('location_id',$id)->where('condition','Equal')->first();
                if(!$shippingRole){
                    $shippingRole = ShippingRole::where('location_id','!=',$id)->where('condition','Not Equal')->first();
                }
                //return ((ceil($weight) - $shippingRole->minimuUnit) * $shippingRole->incrementPerUnit) + $shippingRole->amount;
                return $shippingRole->amount;
            }
            else{
                $shippingRole = ShippingRole::where('condition','Not Equal')->first();
                //return ((ceil($weight) - 1) * 10) + 60;
                return $shippingRole->amount;
            }
        }
        
        //return ShippingRole::where('id',$id)->first()->amount;
    }

    public function shipingAmountAdmin($id = null)
    {
        $weight = 0;       
