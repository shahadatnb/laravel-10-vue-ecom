...
const cart = reactive({
    ...
    async placeOrder(name, phone, email, state, address, shipping_method, payment_method, txn_id, note){
        basic.loading = true
        ...
        try {
            const response = await fetch(`${basic.serverUrl}/api/placeOrderNonAuth`, {
                ...
                body: JSON.stringify({ name, phone, email, state, address, shipping_method, payment_method, txn_id, note, products, shippingCost:this.shippingCost, totalPrice: this.totalPrice })
            })
    ...
    shippingMethod(shipping_method){
        if(shipping_method == 'sundarban_point'){
            this.shippingCost = basic.settings.shipping_sundarban_point
        }else if(shipping_method == 'home_delivery'){
            this.shippingCost = basic.settings.shipping_home_delivery
        }else{
            this.shippingCost = basic.settings.shipping_sundarban_point
        }
