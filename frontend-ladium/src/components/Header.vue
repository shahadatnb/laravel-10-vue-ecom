...
<template>
          ...
          >
            ...
            <div class="xl:col-span-4 flex items-center gap-5 justify-between">
              ...
              <div class="flex items-center justify-between gap-5">
                <div @click="searchToggle" class="cursor-pointer hover:text-[#0d6efd]">
                ...
                <div class="flex items-center gap-2">
                    <router-link to="/dashboard/wishlist" class=" hover:text-[#0d6efd] relative mr-3">
                        <font-awesome-icon :icon="['fas', 'heart']" />
                        <span class="absolute -right-2 -top-3">{{ wishlist.totalWishlistItems }}</span>                        
                    </router-link>
                    <router-link to="/cart" class="relative  hover:text-[#0d6efd]">
      ...
      >
        <ul class="flex flex-col gap-0 justify-center px-3" v-if="basic.settings.menus">
          <li @click="mobileMenuToggle" v-for="menu in basic.settings.menus.main" :key="menu.id" class="menu-item hover:bg-[#ddd]">
            <router-link :to="menu.menu_url" class="w-full block p-2">{{menu.lebel}}</router-link>
          </li>
        </ul>
      </nav>
      <transition name="fade">
      <div v-if="SearchBox" class="fixed inset-0 z-50">
        <div @click="searchToggle" class="absolute bg-black opacity-70 inset-0 z-0">
        </div>
